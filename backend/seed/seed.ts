...
import { TransactionItem } from '../src/transactions/entities/transaction-item.entity';
import { InventoryLog, InventoryType } from '../src/inventory/entities/inventory.entity';
...
async function seed() {
  ...
  await transactionItemRepo.query('DELETE FROM transaction_items');
  await inventoryLogRepo.query('DELETE FROM inventory_logs');
  await transactionRepo.query('DELETE FROM transactions');
  ...
  const users = await userRepo.save([
    ...
    userRepo.create({
      ...
      passwordHash: passwordHash('cashier123'),
      role: Role.CASHIER,
      isActive: true,
    }),
    userRepo.create({
    ...
    userRepo.create({
      ...
      passwordHash: passwordHash('warehouse123'),
      role: Role.WAREHOUSE,
      isActive: true,
    }),
  ]);
  const [adminUser, cashier1, cashier2, warehouseUser] = users;
  ...
  const cashiers = [adminUser, cashier1, cashier2];
  ...
  for (const def of txDefs) {
    ...
    await patchCreatedAt('transactions', transaction.id, def.createdAt);
    ...
    for (const itemDef of def.items) {
      ...
      const invLog = await inventoryLogRepo.save(
      ...
      await patchCreatedAt('inventory_logs', invLog.id, def.createdAt);
    }

    console.log(
      `  Transaction ${invoiceNo}: ${def.items.length} item(s), Rp${total.toLocaleString('id-ID')}`,
    );
  }
